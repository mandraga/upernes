...
tile:
	...
	lda #$80
	sta VMAINC
	; CHR data is at $0000 and is 8k in size ie up to $1000W
	; Tile data is at $2000W and is 2k in size
	lda #$00
	sta VMADDL		; Word address
	lda #$20
	sta VMADDH
...
quarth:
	...
	lda #$20		; (1k word segment $3000 / $400)=$06 << 2
	sta BG1SC
...
palconv3:
	...
	;; -----------------------------------------------
	; Load the 2KB ASCII table CHR data at VRAM $1000W
        ;; -----------------------------------------------
...
; 	plb			; Restore data bank
	...
	;; -----------------------------------------------
	; BG3 CHR address
	; Ascii CHR data is at $1000W, this is from tile
	; $C0 to tile $FF ($C0 + 64 = $C0 + $40 = $100)
	lda #$01		; 0x1000W -> first 4kWord=8k segment (512 tiles, the asci table starts at 0x1000W: tile 384/0x180
	...
	; $1800 >> 11 << 2= $58
        lda #$18                ; (1k word segment $1800 / $400)=$6 << 2
        sta BG3SC
...
; 	;Creates a test tile map for BG3
 	...
 	lda #$80
 	sta VMAINC
; 	; Tile map is at $18000
 	lda #$00
 	sta VMADDL		; Word address (= $1800)
 	lda #$18
 	sta VMADDH
; 	; CHR data is at $1000word
 	lda #$00		; Tile number
 	ldy #$00		; nes equivalent sprite bank
 	ldx #$0400		; Total number of tiles 32x32
 quarthBG3:
 	sta VMDATAL		; Flip tile & palette selection: 0
 	sty VMDATAH		; tile number
